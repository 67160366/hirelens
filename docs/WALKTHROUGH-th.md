...
- **เปลี่ยนรหัสผ่านแล้วไล่ทุกเครื่องออกได้แล้ว** (2026-08-18, migration `0012`) —
  เดิมทำไม่ได้เพราะ denylist เก็บแค่ token ที่ตายแล้ว ไม่เก็บที่ยังมีชีวิต จึงไม่มีรายการให้ไล่
  คำตอบคือ **token epoch ต่อบัญชี**: token ทุกใบพก "รุ่น" ที่ตัวเองถูกออกมา, row พก
  รุ่นปัจจุบัน, ไม่ตรงกัน = ปฏิเสธ ดังนั้นบวกเลขจำนวนเต็มหนึ่งตัวก็จบทุก session ที่ไม่มี
  อะไรเคยบันทึกไว้ — นี่คือเหตุผลที่เลือก epoch แทน session registry ซึ่งต้องเขียน row
  ทุกครั้งที่ login และต้องมีตัวกวาดเพิ่มอีกตัว
  รายละเอียดที่จำไว้จะได้ไม่พลาด: `assert_live` **บังคับ**ให้ส่ง row ของบัญชีเข้าไปด้วย
  เพื่อให้กฎ "เรียกคู่กัน" ยังเป็นสองอย่าง ไม่กลายเป็นสาม (mypy จะฟ้องถ้าไม่โหลด row);
  token เก่าที่ไม่มี claim `epoch` อ่านเป็น 0 ซึ่งตรงกับค่า backfill จึงยังใช้ได้ต่อ
  แต่ตายพร้อมกันหมดตอนเปลี่ยนรหัสผ่านครั้งแรก; และเทียบด้วย `!=` ไม่ใช่ `<`
  เพราะ token ที่อ้าง epoch สูงเกินจริงคือของปลอม
- **บทเรียนจาก mutation pass ครั้งนี้ สวนกับ precedent ของโปรเจคเอง และถูกแล้ว**:
  ลบ `revoke(..., PASSWORD_CHANGED)` ออก ไม่มี test พังเลย เพราะ epoch ปฏิเสธ token
  ก่อนอยู่แล้ว แต่ **เก็บไว้** ต่างจาก `is_revoked` fast path และ separator reset ใน
  `geometry.py` ที่ถูกลบ — เพราะสองอันนั้นเป็น *guard* ที่พังไม่ได้ ส่วนอันนี้เป็น
  *บันทึก* และการบวก epoch ไม่ทิ้งประวัติอะไรเลย ถ้าลบทิ้ง การเปลี่ยนรหัสผ่านจะเป็น
  การเพิกถอนอย่างเดียวที่ operator มองไม่เห็นเหตุผล ตอนนี้มี test pin ไว้แล้ว
  รูปทั่วไป: "ลบแล้วไม่มี test พัง" แยก dead code ออกจาก *untested* code ได้ก็ต่อเมื่อ
  ถามต่อว่าบรรทัดนั้นมีไว้ทำอะไร
